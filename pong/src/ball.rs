...
/// Направление подачи задаётся знаком, как и направление ракетки: вправо — положительное, влево —
/// отрицательное.
const SERVE_LEFT: f32 = -1.0;
...
pub struct Ball {
    ...
    pub velocity_y: f32,
    pub size: f32,
    /// Направление последней подачи. Хранится отдельно от скорости, потому что за раунд мяч меняет
    /// направление много раз, а чередование подач (D-11) отсчитывается от подачи к подаче, а не от
    /// того, куда мяч летел в момент гола.
    serve_direction: f32,
}

impl Ball {
    /// Мяч в центре поля с первой подачей. Начало матча и возобновление после гола — одно и то же
    /// правило, поэтому конструктор выражен через сброс: правило живёт в одном месте и покрыто его
    /// тестом. Положение и скорость здесь только заготовка, их задаёт сам сброс.
    pub fn new(field: Field) -> Self {
        let mut ball = Self {
            ...
            velocity_y: 0.0,
            size: SIZE,
            // Первая подача уходит вправо, а сброс меняет направление на противоположное, поэтому
            // «предыдущей» подачей записана левая.
            serve_direction: SERVE_LEFT,
        };
    ...
    /// Возврат мяча в центр поля после гола.
    pub fn reset(&mut self, field: Field) {
        // Позиция мяча — его левый верхний угол, поэтому в центре поля стоит не она, а середина мяча.
        self.x = (field.width - self.size) / 2.0;
        self.y = (field.height - self.size) / 2.0;

        // Подача не выбирается случайно (D-11): она уходит в сторону, противоположную предыдущей
        // подаче, поэтому чередование остаётся детерминированным и проверяется unit-тестом.
        self.serve_direction = -self.serve_direction;
        self.velocity_x = SPEED_X * self.serve_direction;
...
mod tests {
    ...
    #[test]
    fn reset_puts_the_ball_in_the_center_of_the_field() {
        let mut served = ball();

        served.reset(field());

        // Центр поля с поправкой на размер мяча: (960 - 20) / 2 = 470 и (600 - 20) / 2 = 290.
        assert!(
            (served.x - 470.0).abs() < TOLERANCE,
            "после сброса мяч не в центре по горизонтали: x = {}, ожидалось 470",
            served.x
        );
        assert!(
            (served.y - 290.0).abs() < TOLERANCE,
            "после сброса мяч не в центре по вертикали: y = {}, ожидалось 290",
            served.y
        );
    }

    /// Направление подачи чередуется, а не выбирается случайно (D-11), поэтому его можно проверить
    /// тестом: два сброса подряд отправляют мяч в противоположные стороны.
    #[test]
    fn serve_direction_alternates_from_reset_to_reset() {
        let mut served = ball();

        served.reset(field());
        let first = served.velocity_x;
        served.reset(field());
        let second = served.velocity_x;

        // Скорости разных знаков дают отрицательное произведение; заодно это исключает нулевую подачу.
        assert!(
            first * second < 0.0,
            "подача не сменила направление: первая velocity_x = {}, вторая velocity_x = {}",
            first,
            second
        );
    }
